// PariltiDetailScreen — bir anıyı parlatan kullanıcıları gösterir.
...
import { getPariltiUsers } from '../services/engagement';
...
export default function PariltiDetailScreen({ navigation, route }) {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const uids = await getPariltiUsers(postId);
            ...
            {rows.map((row) => {
              ...
              return (
                ...
                >
                  ...
                  <Ionicons name="sparkles" size={18} color={sunset.orange} />
                </Pressable>
              );
            })}
          </View>
